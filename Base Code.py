	...
	def update(self):
		if(game.key("c")):
			...
			for b in blocksManager.ActiveBlocks:	
					if(self.collide(b)):
						return;
			blocksManager.AddBlockTile(Block(self.RelVector.x,self.RelVector.y,1),True);
#########################	
class BlocksManager:
	def __init__(self,manager):
		self.manager = manager;
...
class Block(Overlay):
	def __init__(self,X,Y,ID):
		Overlay.__init__(self,Vector2(X,Y),True);
		self.image = 63;
		self.size = size;
		self.ID = ID;
...
class Generator:
	...
	def GenerateFillarOfDirt(self,BlockY,BlockX,num):
		if(num-1==BlockY):
			self.engine.AddBlockTile(Block(size * BlockX, size * BlockY, 1),False);
			...
			self.engine.AddBlockTile(Block(size * BlockX, size * BlockY, 2),False);
	def GenerateTree(self,X,Y):
		for i in range(Y+1,Y+4):
       			self.engine.AddBlockTile(Block(size * X, size * i, 4),False);
		self.engine.AddBlockTile(Block(size * X, size * (Y+4), 5),False);
		self.engine.AddBlockTile(Block(size * (X-1), size * (Y+3), 5),False);
		self.engine.AddBlockTile(Block(size * (X+1), size * (Y +3), 5),False);
		self.CanGenerateTree = 0;
	def CreateUnderGround(self,size):
		for x in range(-size,size):
			self.GenerateStoneCollumn(x);
	def GenerateStoneCollumn(self,x):
		for y in range(-1,-15,-1):
			self.engine.AddBlockTile(Block(size*x,size*y, 3),False);
#################
class IDProcessor:
...
generator = Generator(blocksManager);
generator.Generate(100)
manager.camera.Move(80,-90)
